...
class WP_GitHub_Theme_Updater {
	...
	/**
	 * Temporarily store the releases fetched from GitHub, so the list is only
	 * loaded once per class instance.
	 *
	 * @var array|null
	 */
	...
	public function set_defaults() {
		...
		// run until config['version'] is known. Everything needing a call to
		// GitHub is deferred to resolve_remote().
	 ...
	 * hour, counted per source IP rather than per site or per theme. It also
	 * cost up to 2s of latency per call, since http_request_timeout is
	 * filtered to 2 seconds.
	 *
	 ...
	 * .github/workflows/nightly.yml, which stamps that version into the theme
	 * header of the zip it ships. A site is therefore on the nightly channel
	 * exactly when its installed version carries that suffix.
	 *
	 * @since 1.1
	...
	public function get_releases() {
		...
		// limit is 60/hour - must not be reported as "no update available".
		// Fall back to the last known good list instead.
	 ...
	 * as an "update", because version_compare() sorts `0.1.0-nightly.20260809`
	 * below `0.1.0`.
	 *
	 * @since 1.1
	...
	public function get_channel_release() {
		...
		foreach ( $releases as $release ) {
			...
			// GitHub source zipball is an unbuilt tree and would ship a theme
	...
	public function api_check( $transient ) {
		...
		if ( empty( $this->config['new_version'] ) ) {
			return $transient;
		}
